...
def conv(x, channels, kernel=4, stride=2, pad=0, pad_type='zero', use_bias=True, sn=False, scope='saconv_0'):
...
def deconv(x, channels, kernel=4, stride=2, padding='SAME', use_bias=True, sn=False, scope='sadeconv_0'):
    with tf.variable_scope(scope):
...
def fully_conneted(x, units, use_bias=True, sn=False, scope='safully_0'):
    with tf.variable_scope(scope):
        x = flatten(x)
...
def resblock(x_init, channels, use_bias=True, is_training=True, sn=False, scope='saresblock'):
    with tf.variable_scope(scope):
...
def batch_norm(x, is_training=True, scope='sabatch_norm'):
