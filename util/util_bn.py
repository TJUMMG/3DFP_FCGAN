...
def resblock(inputs,out_channels,is_training,name):
    with tf.variable_scope(name):
        ...
        x = conv2d(inputs,'conv1',out_channels,bn=True,is_training=is_training,activation=True)
        x = conv2d(x,'conv2',out_channels,bn=False,is_training=is_training,activation=False)

        if in_channels!=out_channels:
            inputs = conv2d(inputs,'conv_fuse',out_channels,bn=True,is_training=is_training,activation=True)
        x = inputs + x
...
def conv_block(x,output_num,is_training,name):
    with tf.variable_scope(name):
        inp = x
        x1 = tf.layers.batch_normalization(x,training=is_training)
        x1 = tf.nn.relu(x1)
        x1 = conv2d(x1,name='conv1',out_channels=output_num//2)

        x2 = tf.layers.batch_normalization(x1,training=is_training)
        x2 = tf.nn.relu(x2)
